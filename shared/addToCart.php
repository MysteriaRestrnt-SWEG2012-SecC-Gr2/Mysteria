...
$DBpath.="/Mysteria/db/connectVar.php";
require_once($DBpath);
...
if(isset($_POST["cardAlone"])){
    ...
    if($result->num_rows==1){
        if($menuQuantity>0 && $menuQuantity>=$foodQuan){
            ...
            echo '<script type ="application/JavaScript"> alert ( "your selection has been added to your cart!"); window.location.assign("http://localhost/Mysteria/public/menu_bar/FoodMenu.php"); </script>'; 
        ...
        foreach($_POST['foodChecked'] as $foods){
                ...
                if($result->num_rows==1){
                    if($menuQuantity>0 && $menuQuantity>=$foodQuan){
                        ...
                        echo '<script type ="application/JavaScript"> alert ( "your selections have been added to your cart!");  window.location.assign("http://localhost/Mysteria/public/menu_bar/FoodMenu.php"); </script>'; 
                    }
                    else{
                        echo '<script type ="application/JavaScript"> alert ( "Sorry! we do not have that many food items in our inventory!\n please decrease the quantity of the food items and order again"); history.back(); </script>';
                    }
    ...
    if($result->num_rows==1){
        if($dbQuan>$foodQuan){
            ...
            echo '<script type ="application/JavaScript"> alert ("the item has been removed from your cart");  window.location.assign("http://localhost/Mysteria/shared/cart.php"); </script>'; 
        }
    ...
    if(!empty($_POST['foodChecked'])){
        foreach($_POST['foodChecked'] as $foods){
            ...
            if($result->num_rows==1){
                if($dbQuan>$foodQuan){
                    ...
                    echo '<script type ="application/JavaScript"> alert ("the selected items have been removed from your cart"); window.location.assign("http://localhost/Mysteria/shared/cart.php");  </script>'; 
                }
                else if($dbQuan<=$foodQuan){
                    $sql="DELETE FROM cart WHERE user_id='$user' and food_id='$foodID'";
                    $result=mysqli_query($connectVariable,$sql);
        ...
        echo '<script type ="application/JavaScript"> alert ( "please check a value/s before removing from cart!"); window.location.assign("http://localhost/Mysteria/shared/cart.php"); </script>';
    }
